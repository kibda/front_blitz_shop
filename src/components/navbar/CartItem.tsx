...
const CartItem = ({ item, cartState }) => {
  const [itemNum, setItemNum] = useState(cartState.cart.find((cartItem) => cartItem.product.id === item.product.id).quantity);
  ...
  const removeItem = async () => {
    console.log("Removing item:", item.product.id);
    try {
      ...
      if (response.status === 200) {
        const newCart = cartState.cart.filter((cartItem) => cartItem.product.id !== item.product.id);
        cartState.setCart(newCart);
      } else {
